...
data class ChampionshipResource(val _id: Int, val name: String, val seasonId: Int) : ResourceSupport() {
    ...
    init {
        add(ControllerLinkBuilder.linkTo(ControllerLinkBuilder.methodOn(SeasonController::class.java).getChampionships(seasonId, UsernamePasswordAuthenticationToken(null, null))).withSelfRel().withRel("championships"))
        add(ControllerLinkBuilder.linkTo(ControllerLinkBuilder.methodOn(ChampionshipController::class.java).getMatches(_id, UsernamePasswordAuthenticationToken(null, null))).withRel("matches"))
    }
}